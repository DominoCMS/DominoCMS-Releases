...
require_once ($config['libRoot'] . 'items/Domino/Entry/DJS1-SF6-D1/List/Field/Picture/controller.php');
class DominoAdminEntryEditFieldCrossPicturesController extends DCBaseController {
    ...
    public function structureData ( $element, $libModule, $moduleData, $dimensions ) {
        ...
        if ( isset($moduleData["developer"]) ) {
            ...
            if ( isset( $moduleData["id"] ) )
                $pics = $util->dominoSql("select * FROM " . $identity['db'] . ".DCDominoPictures mt JOIN " . $identity['db'] . ".DCDominoSiteCrossPictures ct ON ( mt.id = ct.idRel ) WHERE ct.entry='".$moduleData["developer"].".".$moduleData["module"].".".$moduleData["id"]."' AND ct.status<3 AND mt.lang='".$lang."';",'fetch_array');
            ...
            $ret = array(
                ...
                'crossLink' => '/'.$identity["username"].'/si/moduli?developer=Domino&module=SiteCrossPictures&id='.$moduleData["developer"].'.'.$moduleData["module"].'.'.$moduleData["id"].'|'
            );
