...
class DominoAdminEntryEditFieldCrossFilesController extends DCBaseController {
    ...
    public function structureData ( $element, $libModule, $moduleData, $dimensions ) {
        ...
        $util = new DCUtil();
            ...
            if ( isset($moduleData["developer"]) ) {
                ...
                $files = $util->dominoSql("select * FROM " . $identity['db'] . ".DCDominoSiteCross WHERE entry='".$moduleData["developer"].".".$moduleData["module"].".".$moduleData["id"]."' AND status<3;",'fetch_array');
                ...
                $ret = array(
                    ...
                    'crossLink' => '/'.$identity["username"].'/si/moduli?developer=Domino&module=SiteCross&id='.$moduleData["developer"].'.'.$moduleData["module"].'.'.$moduleData["id"].'|',
                    'fileLink' => '/'.$identity["username"].'/si/moduli?developer=Domino&module=Files&id='
                );
