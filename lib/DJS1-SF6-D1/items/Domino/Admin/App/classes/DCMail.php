...
class DCMail {
    ...
    public function sendSystemMail ( $data ) {
        ...
        if ( isset( $data["subject"] ) && isset( $data["message"] ) ) {
            ...
            $translate = $translate->showTranslate( array("website_automatic_reply_message","message_missed_request","sent_domino_message","automatic_system_message"));
            ...
            if ( $site['email']['username'] && $site['email']['password'] && $site['email']['smtp'] ) {
                require_once $config['libRoot'] . 'items/' . 'Swiftmailer/Swiftmailer/swift_required.php';
